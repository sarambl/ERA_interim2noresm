...
# Needs files date_info_day.py, modify_vct_modify.py and weights.nc in same folder
...
# You also need to use the topography for 
...
#cdo -s vct res_file.nc > vct_wrong.txt #Commented out by Moa, it does for some reason work with the new cdo
# The modify_vct_modified.py now instead uses an old file called vct_wrong_IH.txt.
#cp ${path}/modify_vct.py .
python modify_vct_modified.py # Change by Moa
